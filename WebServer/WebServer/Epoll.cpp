...
typedef shared_ptr<Channel> SP_Channel;   //共享指针管理Channel
//构造、析构
...
void Epoll::epoll_add(SP_Channel request, int timeout) {
  ...
  if (timeout > 0) {  //如何对这个channel设置了超时时间，则设置定时器
  ...
  fd2chan_[fd] = request; //以fd为索引，映射对应的 channel 指针
  if (epoll_ctl(epollFd_, EPOLL_CTL_ADD, fd, &event) < 0) {   //将fd和感兴趣事件注册到epoll底层的红黑树中进行监听
