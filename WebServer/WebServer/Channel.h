...
class Channel {
 private:
  typedef std::function<void()> CallBack;   //std::function是一个可调用对象包装器，是一个类模板，可以容纳除了类成员函数指针之外的所有可调用对象，void()表示无返回值，函数参数为空
  EventLoop *loop_; //当前Channle属于的EentLoop
  int fd_;  //监听的文件描述符
  __uint32_t events_; //该Channel监听的文件描述符感兴趣的事件类型集合（可读、可写等等）
  __uint32_t revents_;  //与events_相应，是监听到的事件类型集合
  __uint32_t lastEvents_; //上一次监听的事件类型集合
 ...
 private:
  ...
  CallBack connHandler_;   //处理新连接到来的回调函数
 ...
 public:
  ...
  void setConnHandler(CallBack &&connHandler) { connHandler_ = connHandler; } //connHandler_是左值（命名的右值引用），connHandler_ = std::move(connHandler)才会触发移动语义
