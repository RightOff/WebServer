...
using namespace std;
//__thread修饰的变量表示该变量是本线程局部的，该变量不会再线程之间共享，其生命周期和各自的线程绑定 
__thread EventLoop* t_loopInThisThread = 0;  //该变量作用是什么？？
...
EventLoop::EventLoop()
    : looping_(false),  //默认没有执行事件循环
      ...
      //将本事件循环和eventfd文件描述符封装到一个Channel中，一个 EventLoop 可以创建多个 Channel ，此处创建的channel是为了注册监听eventfd事件从而唤醒该EventLoop
      pwakeupChannel_(new Channel(this, wakeupFd_)) { 
  if (t_loopInThisThread) {
    // LOG << "Another EventLoop " << t_loopInThisThread << " exists in this
    // thread " << threadId_;
  } else {
    t_loopInThisThread = this;  //指向本线程，遵循One loop per thread
  }
  // pwakeupChannel_->setEvents(EPOLLIN | EPOLLET | EPOLLONESHOT);，注册一个 channel 监听此事件循环EventLoop，设置其回调函数。
  pwakeupChannel_->setEvents(EPOLLIN | EPOLLET);  //可读、边缘触发
  pwakeupChannel_->setReadHandler(bind(&EventLoop::handleRead, this));  //向 Channel 注册可读事件的回调函数
  pwakeupChannel_->setConnHandler(bind(&EventLoop::handleConn, this));  //向 Channel 注册新连接到来的回调函数，bind 将this参数绑定到handleConnh函数上，返回值是一个函数对象，在这里为临时对象
  poller_->epoll_add(pwakeupChannel_, 0); //将Channel注册到epoll中进行监听 
}
//回调函数
